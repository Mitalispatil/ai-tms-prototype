...
def extract_text_from_json(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return json.dumps(data, indent=2)  # readable format
...
def load_old_testcases(path="old_testcases.json") -> str:
    return load_file(path)

def save_adapted_testcases(adapted: str, path="adapted_testcases.json"):
...
def main():
    # Step 1: BRD diff
    old_brd = extract_text_from_pdf("old_brd.pdf")
    new_brd = extract_text_from_pdf("new_brd.pdf")
    brd_diff = generate_diff(old_brd, new_brd)

    # Step 2: Code diff from Git
    os.system("git fetch origin main")
    os.system("git diff origin/main -- '*.py' '*.js' '*.jsx' > code_diff.txt")
    code_diff = load_file("code_diff.txt")

    # Step 3: Figma diff (assumed exported as JSON)
    old_figma = extract_text_from_json("old_figma.json")
    new_figma = extract_text_from_json("new_figma.json")
